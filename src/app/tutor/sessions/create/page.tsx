...
const parseApiError = (error: any): string => {
  ...
  if (errorMessage.includes("Validation failed")) {
    // Extract the first validation error for user-friendly display
...
export default function CreateSessionPage() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      if (response.success) {
        ...
        // If courseId is provided via URL, select it
        if (courseId) {
        ...
        // Otherwise select first course if available
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Prepare final data with schedule configs
      const finalData = {
        ...formData,
      ...
      if (response.success) {
        ...
        // Handle case where response.success is false but no error was thrown
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            {courseId ? (
              // Show selected course info without dropdown
              ...
              // Show dropdown only when no courseId is provided
              <select
                name="tutor_course_id"
            ...
            {/* Hidden input for courseId when it's from URL */}
            {courseId && (
              <input
                type="hidden"
                name="tutor_course_id"
