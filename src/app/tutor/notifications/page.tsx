...
import tutorNotificationsApi, {
  Notification,
} from "@/lib/api/tutor-notifications";
...
export default function TutorNotificationsPage() {
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      const response = await tutorNotificationsApi.getNotifications(params);
  ...
  const loadUnreadCount = async () => {
    try {
      const response = await tutorNotificationsApi.getUnreadCount();
      if (response.success && response.data) {
  ...
  const loadPreferences = async () => {
    try {
      const response = await tutorNotificationsApi.getPreferences();
      if (response.success && response.data) {
  ...
  const handleArchive = async (notificationId: number) => {
    try {
      await tutorNotificationsApi.archiveNotification(notificationId);
  ...
  const handleDelete = async (notificationId: number) => {
    ...
    try {
      await tutorNotificationsApi.deleteNotification(notificationId);
  ...
  const handleUpdatePreference = async (
    ...
    try {
      await tutorNotificationsApi.updatePreference(type, { [field]: value });
  ...
  // Get notification icon based on type (tutor specific)
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {/* Preferences Panel (same as student but with tutor-specific types) */}
      {showPreferences && (
        <div className="mb-6 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
          <div className="flex justify-between items-center mb-4">
      ...
      {/* Main content with split view (similar structure as student) */}
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        {/* Notification Detail (similar structure as student) */}
